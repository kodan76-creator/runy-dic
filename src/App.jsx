...
function App() {
  ...
  useEffect(() => {
    const adm = localStorage.getItem('adminUser')
    const cur = localStorage.getItem('currentUser')
    if (adm) { try { setUser({ ...JSON.parse(adm), role: 'admin' }) } catch{} }
    else if (cur) { try { setUser({ ...JSON.parse(cur), role: 'user' }) } catch{} }
    setAuthLoading(false)
  }, [])
  
  const handleUserLogin = (u) => setUser({ ...u, role: u.role || 'user' })
  const handleLogout = () => { localStorage.removeItem('currentUser'); localStorage.removeItem('adminUser'); setUser(null) }
