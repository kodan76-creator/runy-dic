...
export const getGitHubFileSha = async (filePath) => {
  try {
...
export const fetchGitHubFile = async (fileName) => {
  try {
...
export const fetchGitHubFileRaw = async (fileName) => {
  try {
    const url = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${fileName}?ref=${GITHUB_BRANCH}&t=${Date.now()}`
    const response = await fetch(url, { headers: getHeaders(), cache: 'no-cache' })
    if (!response.ok) {
...
export const updateGitHubFile = async (fileName, newData, currentSha) => {
  try {
...
export const withWriteLock = (fileName, task) => {
...
export const isRetryableGitHubError = (error) => {
