...
describe('getDictionaryFileNameForEmail', () => {
  it('returns the personal dictionary inside the user folder', () => {
    expect(getDictionaryFileNameForEmail('User@Example.COM')).toBe('public/users/user_example.com/dictionary.json')
    expect(getDictionaryFileNameForEmail('ivan.petrov@mail.ru')).toBe('public/users/ivan.petrov_mail.ru/dictionary.json')
    expect(getDictionaryFileNameForEmail('ivan+tag@mail.ru')).toBe('public/users/ivan_tag_mail.ru/dictionary.json')
  })
...
describe('resolveDictionaryFile', () => {
  ...
  it('returns personal file in the user folder for regular users', () => {
    expect(resolveDictionaryFile({ role: 'user', paid: false, email: 'Vasya@Mail.ru' })).toBe('public/users/vasya_mail.ru/dictionary.json')
  })

  it('handles string and empty input', () => {
    expect(resolveDictionaryFile('user@mail.ru')).toBe('public/users/user_mail.ru/dictionary.json')
